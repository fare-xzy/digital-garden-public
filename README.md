...
完整对照表 + tag 字典见 [Index/Tags](Index/Tags.html)。

## 如何写笔记

1. `Cmd+N` 新建一篇 → 默认套 `Templates/Note.md`
2. 改 frontmatter：`title:` / `date:` / `tags:` / `publish: true|false` / `status:`
3. 主体用 markdown；双向链接用 `[另一篇笔记](另一篇笔记.html)` 或 `[显示文字](Note.html)`
4. 写完跑 `make publish`（公开稿）或 `make sweep`（找孤立）

更详细的"为什么要这样"见 [Getting Started](Notes/getting-started.html)。
